...
export function AppSidebar({
  navItems,
  secondaryNavItems,
  userNavProps: _userNavProps,
  ...props
}: AppSidebarProps) {
  return (
    <Sidebar collapsible="icon" variant="sidebar" {...props}>
      <SidebarHeader className="min-h-0 border-b pt-0">
        <div className="flex min-h-12 items-center gap-2 pl-2 group-data-[collapsible=icon]:p-3">
          <LangfuseLogo />
        </div>
        <DemoBadge />
      </SidebarHeader>
      ...
      <SidebarGroupContent>
        <SidebarMenu>
          <SidebarMenuItem>
            ...
            >
              <Link
                href="http://localhost:3001/docs/demo"
                target="_blank"
          ...
          <SidebarMenuItem>
            <SidebarMenuButton asChild tooltip="Your Agent Guard Organizations">
              <Link href="/">
                <Grid2X2 className="h-4 w-4" />
                <span>Your Agent Guard Orgs</span>
              </Link>
            </SidebarMenuButton>
          </SidebarMenuItem>
        </SidebarMenu>
